...
Entries in the *duration* column are calculated from the values in the from and to columns. By adding `=B3-A3` to the third row, selecting all the other rows in this column except the header rows and pressing <kbd>CTRL</kbd>+<kbd>D</kbd> the desired duration will be calculated for all data rows.
