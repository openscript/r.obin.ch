...
1. Install `lvm2`, `vim` and `btrfs-progs` with `pacman -Sy lvm2 vim btrfs-progs`
...
1. Install firmware updated `pacman -S fwupd`
...
1. Install fingerprint reader `pacman -S fprintd`
   - Scan fingerprint in Gnome Settings

## Tools

1. Install docker (Container runtime) `pacman -S docker`
